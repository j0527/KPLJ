...
class MainPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 1. 앱 실행시 stateless라서 FutureBuilder를 써서 _initLocation()라는 함수를 쓰고 거기서 위치 권한 사용동의 창을 띄움
    // 2. 동의시에는 위치를 geolocator 패키지의 position이라는 변수에 저장됨 ex): Latitude: 37.49474, Longitude: 127.030034 이런식이라서 position.Latitude 하면 사용됨
    // 3. _initLocation() 함수에서는 위치를 가져오고 getXmodel부분의 calculateDistance()함수를 호출해서 계산함 (거리 계산은 getXmodel에 설명 써놓음)
    // 4. 거리가 계산되면 FutureBuilder 내부에 calculateDistance를 호출하는 인스턴스로 선언해놓은 calcDistance에 거리 값을 저장
    // 5. carouselItems의 userDistance에 거리를 업데이트
    // 6. imageSlider위젯의 userInfoList부분에 carouselItems를 넣어줘서 화면에 띄워주게됨
        ...
        child: FutureBuilder<Position?>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.done) {
              if (snapshot.hasData) {
                // final position = snapshot.data; // geolocator의 position
                // locationController.myLocation.value = position;
                ...
                // 3항 연산자로 1000을 넘으면 km로 변환하기 위해서 1000으로 나누고 단위도 km로 바꿈
                reciveUserDistance =
...
void _showPermissionDeniedDialog() {
  ...
  Get.defaultDialog(
    title: '권한 거부됨',
    middleText: '앱 사용을 위해 위치 권한이 필요합니다. 권한을 허용하지 않을 시 서비스를 이용할 수 없습니다.',
